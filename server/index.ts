...
import { envProtectionMiddleware, setupFileSystemProtection, testEnvProtection } from './env-protection';
import * as crypto from "crypto";

const app = express();

// Initialize .env protection systems
setupFileSystemProtection();
testEnvProtection();

// Apply .env protection middleware (first middleware for maximum security)
app.use(envProtectionMiddleware);
...
// SECURITY: Enhanced session configuration with strict single-session enforcement
...
(async () => {
  // Run password migration on startup to ensure security
  try {
